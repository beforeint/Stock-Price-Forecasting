...
dataset_path = '/home1/pz29075/Stock Price Forecasting/StockMixer/dataset/' + market_name
if market_name == "SP500":
    data = np.load('/home1/pz29075/Stock Price Forecasting/StockMixer/dataset/SP500/SP500.npy')
    data = data[:, 915:, :]
    price_data = data[:, :, -1]
    ...
    # 수정 1: row의 시작점을 1이 아닌 steps로 변경 (미래 데이터 참조 방지)
    for ticket in range(0, data.shape[0]):
        for row in range(steps, data.shape[1]):
            # 수정 2: gt_dataticket][row] -> gt_data[ticket][row] 대괄호 추가
